...
for i in {0..7499..500}
    do
        sbatch --qos=normal --requeue -p gpu,rush -N1 -n1 --time=1000:00:00 --ntasks-per-core=4 --mem-per-cpu=32G --gres=gpu:a6000:1 --wrap "python sample_model_a.py --dataset_name hendrycks/competition_math --dataset_split train --model models/mistralai/Mistral-7B-v0.1-model-a-game --num_samples 30 --start $i --end $((${i}+500)) --nofewshot"
    done
