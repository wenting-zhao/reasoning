...
def format_example(example, include_answer=True, chain=False):
    user = "You will be given an extremely difficult problem and its decomposed sub-problems. Your job is to derive an answer by finding solutions to the sub-problems. Please highlight your solution with \\boxed{number} where number is the numerical answer without unit.\n\nProblem: " + example['problem']
    if include_answer:
        ...
        assistant = ""
        for q, a in example['steps']:
            assistant += f"Sub-problem: {q}\nSolution to the sub-problem: {a}\n"
        assistant += "Therefore, the answer is \\boxed{"+example['answer']+"}."
        prompt +=  [{"role": "assistant", "content": assistant}]
        return prompt
    all_prompts = []
    for text in example['model-a']:
        curr = format_qa(text)
        curr_qa = user
        for q, a in curr:
            curr_qa += f"\nSub-problem: {q}"
        prompt = [{"role": "user", "content": curr_qa}]
...
def gen_prompt(test_example, fewshot_examples):
    ...
    prompt = [deepcopy(prompt) for _ in out]
...
def main():
    ...
    out_name = args.dataset_name.replace("model-a-samples", "model-b-samples")
